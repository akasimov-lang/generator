...
function ProjectConfigEditor({ siteId, api }: { siteId: string; api: Api }) {
  ...
  return <div className="autoReglueSettings">{error && <div className="notice" role="alert">{error}</div>}
    ...
    <div className="networkSection">
      ...
      <label className="checkboxRow"><input type="checkbox" disabled={!draft.create_subdomains} checked={!!draft.subdomain_add_casino} onChange={e => change({ subdomain_add_casino: e.target.checked })} /> Добавлять варианты со словом casino: pincocasinoaz.example.com</label>
      ...
      <p className="muted">Один запуск создаёт один поддомен существующего корня. GEO и язык берутся из проекта, бренд — из поля «Бренд». Имена проверяются по кешу, актуальной сетке и истории. В смешанном режиме порядок вариантов различается по проектам. Выбранное имя фиксируется в плане и не меняется при повторе задачи. Если бренд неизвестен или имена закончились, запуск останавливается с объяснением. AMP исключены.</p>
