              ...
              return [
                ...
                item.site_id ? (
                  <SearchableSelect
                    ...
                    options={[{ value: "", label: "Выберите раздел" }, ...sections.map((section) => ({ value: section.id, label: section.name, description: section.path }))]}
                    disabled={itemBusy}
                  ...
                  <span>
                    ...
                    <span className="projectCanonValue">
                      ...
                      <span className="projectCanonActions">
                        <a className="projectCanonOpenButton" href={selectedSite.base_url} target="_blank" rel="noreferrer" data-tooltip={`Открыть MAIN-сайт: ${selectedSite.cache_canon || selectedSite.base_url}`} aria-label={`Открыть MAIN-сайт ${selectedSite.cache_canon || selectedSite.base_url}`}>
                          <ExternalLink size={14} />
                        </a>
                        <button className="projectCanonCopyButton" type="button" onClick={copyCanon} data-tooltip={canonCopied ? "Адрес MAIN скопирован" : `Скопировать MAIN: ${selectedSite.cache_canon || selectedSite.base_url}`} aria-label="Копировать адрес MAIN">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          data-tooltip={`Открыть проект ${selectedSite.name} в универсальной админке`}
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          data-tooltip={`Открыть проект ${selectedSite.name} в web-dev`}
        ...
        <form className="formGrid" onSubmit={createTask}>
          ...
          <label>
            ...
            <SearchableSelect
              ...
              options={[{ value: "", label: "Выбрать позже" }, ...sections.map((section) => ({ value: section.id, label: section.name, description: section.path }))]}
...
function ProjectContentPanel({ api, site, content: allContent, sections, onChanged }: ViewProps & { site: Site; content: ContentItem[]; sections: Section[] }) {
  ...
  const contentTargetOptions = React.useMemo(() => [
    ...
    ...sections.filter((section) => section.sync_status !== "external_deleted").map((section) => ({
      value: section.id,
      label: section.name, description: section.path
    })),
  ...
  return (
    <section className="viewStack">
      ...
      {mode === "workflow" && workflowSection === "backlog" ? <DataPanel id="workspace-section-backlog" className="publicationBacklogPanel" collapseKey="publication-backlog" title={<span className="publicationBacklogTitle"><AlertTriangle size={18} /> Ошибки публикации · {publicationBacklog.length}</span>}>
        ...
        <ResponsiveTable
          columns={["Тема", "Меню", "Ошибка", "Время", "Статус", "Действия"]}
          rows={publicationBacklog.map((item) => {
            ...
            return [
              ...
              <PublicationStatus status={item.status} statusCode={item.last_publication_status_code} />,
              <button className="button compact primary publishImmediatelyButton" type="button" onClick={() => void publishImmediately(item)} disabled={publishingNowId === item.id}><Send size={14} /> {publishingNowId === item.id ? "Проверяем…" : "Отправить ещё раз"}</button>
            ];
          })}
...
function ProjectMenuPanel({ api, site, sections, content, logs, menuCapabilities, onAddContent, onChanged }: ViewProps & { site: Site; sections: Section[]; content: ContentItem[]; logs: PublicationLog[]; menuCapabilities: MenuCapabilities | null; onAddContent: (section: Section) => void }) {
  ...
  async function deleteNestedPage(item: ContentItem) {
    ...
    try {
      ...
      setMenuNestingNotice(error instanceof Error ? error.message : "Не удалось удалить вложенную страницу");
    } finally {
      setDeletingNestedPageId(null);
    }
  }

  async function retryNestedPagePublication(item: ContentItem) {
    setRetryingNestedPageId(item.id);
    setFormError("");
    setMenuNestingNotice("");
    try {
      await api<ContentItem>(`/content/${item.id}/publish-immediately`, { method: "POST" });
      ...
      setMenuNestingNotice(error instanceof Error ? error.message : "Не удалось повторно отправить страницу");
    } finally {
      setRetryingNestedPageId(null);
    }
  }
  ...
  async function openTreeSectionEdit(targetMenuType: "header" | "footer", item: MenuPreviewItem, existingSection: Section | undefined, treeKey: string) {
    setOpeningTreeEditKey(treeKey);
    setFormError("");
    try {
      const result = existingSection
        ? { section: existingSection, created: false }
        : await api<{ section: Section; created: boolean }>(`/sites/${site.id}/sections/adopt`, {
            ...
            body: JSON.stringify({
              external_id: item.externalId,
              name: item.title,
              path: normalizedTreePath(item.path),
              menu_type: targetMenuType,
              parent_id: null
            })
          });
      ...
      setFormError(error instanceof Error ? error.message : "Не удалось открыть редактирование пункта меню");
    } finally {
      setOpeningTreeEditKey(null);
    }
  }
  ...
  async function saveSectionEdit(section: Section, syncImmediately = false) {
    ...
    try {
      ...
      if (syncImmediately) {
        ...
        if (!syncResult.success) {
          ...
          throw new Error(failure?.error || "Изменения сохранены, но меню проекта не удалось обновить");
        }
      }
      ...
      if (syncImmediately) await onChanged();
    } finally {
      setSavingSectionEdit(false);
    }
  }

  async function toggleMenuReview(menuType: "header" | "footer", item: MenuPreviewItem, section?: Section) {
    setReviewSaving(true);
    setFormError("");
    setMenuNestingNotice("");
    try {
      await api(`/sites/${site.id}/sections/review`, { method: "PATCH", body: JSON.stringify({
        external_id: item.externalId, name: item.title, path: item.path, menu_type: menuType,
        is_review: !section?.is_review,
      }) });
      await onChanged();
    } catch (error) { setMenuNestingNotice(error instanceof Error ? error.message : "Не удалось изменить Review"); }
    finally { setReviewSaving(false); }
  }

  async function deleteMenuBranch(menuType: "header" | "footer", item: MenuPreviewItem) {
    if (!window.confirm(`Удалить пункт «${item.title}» и все вложенные пункты из ${menuType === "header" ? "Header" : "Footer"}? Остальные ветки и тексты страниц сохранятся.`)) return;
    setDeletingSectionId(`${menuType}:${item.externalId}`);
    setFormError("");
    setMenuNestingNotice("");
    try {
      await api(`/sites/${site.id}/menu/${menuType}/branch`, { method: "DELETE", body: JSON.stringify({ path: item.path, external_id: item.externalId }) });
      cancelSectionEdit();
      setUpdatedAt(new Date().toISOString());
      await onChanged();
    } catch (error) {
      setMenuNestingNotice(error instanceof Error ? error.message : "Не удалось удалить ветку меню");
        ...
        {persistedSections.length || publicationPages.length ? <ResponsiveTable
          ...
          rows={[...persistedSections.map((section) => {
            ...
            return [
              ...
              item.status === "published" ? <div className="menuSectionEditActions"><a className="button compact secondary" href={projectContentSiteUrl(site, item)} target="_blank" rel="noreferrer" title="Открыть опубликованную страницу"><ExternalLink size={14} /> URL</a><button className="button compact danger" type="button" onClick={() => void deleteNestedPage(item)} disabled={deletingNestedPageId === item.id} title={`Удалить опубликованную страницу${section ? ` из «${section.name}»` : ""}`}><Trash2 size={14} /> {deletingNestedPageId === item.id ? "Удаляем" : "Удалить"}</button></div> : <button className="button compact primary" type="button" onClick={() => void retryNestedPagePublication(item)} disabled={retryingNestedPageId === item.id}><Send size={14} /> {retryingNestedPageId === item.id ? "Проверяем…" : "Отправить ещё раз"}</button>
            ];
          })]}
        /> : null}
...
type AdminPublishedItem = {
  ...
  title: string; slug: string; published_url: string | null; words: number; published_at: string | null;
...
function AdminPublishedView({ api, sites, onChanged, onOpenProject }: ViewProps & { sites: Site[]; onOpenProject: (name: string) => void }) {
  const [items, setItems] = React.useState<AdminPublishedItem[]>([]);
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    setError("");
    api<{ items: AdminPublishedItem[]; total: number }>(`/admin/published?offset=${offset}&limit=${limit}${siteId ? `&site_id=${encodeURIComponent(siteId)}` : ""}`)
      .then((data) => { if (!cancelled) { setItems(data.items); setTotal(data.total); } })
      .catch((reason) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "Не удалось загрузить публикации"); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [api, offset, siteId, refresh]);
  React.useEffect(() => {
  ...
  async function openPreview(id: string) {
    ...
    setError("");
    try { setPreview(await api<ContentItem>(`/content/${id}`)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось открыть текст"); }
    finally { setPreviewLoading(""); }
  }
  return <section className="viewStack"><DataPanel title={`Опубликованные тексты · ${total}`} actions={<button type="button" className="button secondary compact" disabled={loading} onClick={() => setRefresh((value) => value + 1)}><RefreshCcw size={15} /> Обновить</button>}>
    <label className="publishedProjectFilter">Проект<select value={siteId} onChange={(event) => { setSiteId(event.target.value); setOffset(0); }}><option value="">Все проекты</option>{sites.map((site) => <option value={site.id} key={site.id}>{site.name}</option>)}</select></label>
    {error ? <p className="formError" role="alert">{error}</p> : null}
    {loading ? <p role="status">Загружаем публикации…</p> : null}
    {!loading && !items.length ? <EmptyState text="Опубликованных текстов пока нет." /> : <ResponsiveTable columns={["Проект", "Тайтл страницы", "Слов", "Дата и время публикации", "Задание на индексацию", "Автор генерации", "Инициатор публикации"]} rows={items.map((item) => [
      ...
      <span title="Слова видимого текста без HTML-разметки и метаданных">{item.words.toLocaleString("ru-RU")}</span>,
      ...
      item.generation_author || "Не сохранён",
      item.publication_author || "Не сохранён",
    ])} />}
    <div className="formActions publishedPagination"><button className="button secondary compact" type="button" disabled={loading || offset === 0} onClick={() => setOffset(Math.max(0, offset - limit))}>Назад</button><span>{total ? `${offset + 1}–${Math.min(offset + limit, total)} из ${total}` : "0"}</span><button className="button secondary compact" type="button" disabled={loading || offset + limit >= total} onClick={() => setOffset(offset + limit)}>Далее</button></div>
...
function buildMenuTree(items: unknown[], sections: Section[]): MenuTreeNode[] {
  ...
  for (const section of sections) {
    ...
    if (matched) {
      ...
      // Pending local edits should be visible immediately, before the next cache
      // refresh confirms the project-side menu update.
      matched.item = { title: section.name, path: section.path, externalId: section.external_id };
...
type SiteMenuPreviewSectionProps = {
  ...
  onToggleReview?: (item: MenuPreviewItem, section?: Section) => void;
  reviewSaving?: boolean;
  onDeleteMenuItem?: (item: MenuPreviewItem) => void;
  deletingMenuItem?: boolean;
  onAddChild?: (item: MenuPreviewItem, section: Section | undefined, treeKey: string) => void;
};

function SiteMenuPreviewSection({ title, items, site, sections = [], content = [], publicationLogs = [], icon, headerAction, action, children, adoptingParentKey, activeParentTreeKey, pagePreviewLoadingKey, editingTreeKey, openingEditKey, editName = "", editPath = "", savingEdit = false, editError = "", deletingNestedPageId, retryingNestedPageId, onPreviewPage, onEditItem, onEditNameChange, onEditPathChange, onSaveEdit, onCancelEdit, onDeletePage, onRetryPage, onAddContent, onAddChild, onDeleteMenuItem, deletingMenuItem = false, onToggleReview, reviewSaving = false }: SiteMenuPreviewSectionProps) {
        ...
        return (
          <li className="siteMenuTreeNode" key={node.key} role="treeitem" aria-expanded={hasChildren ? !collapsed : undefined}>
            <div className="siteMenuTreeRow">
              ...
              {onAddChild ? <button className="siteMenuAddChildButton" type="button" onClick={() => onAddChild(node.item, node.section, node.key)} disabled={Boolean(adoptingParentKey)} title={`Добавить вложенный пункт в «${node.item.title}»`}><span className="buttonPlusIcon"><Plus size={15} /></span> {adoptingParentKey === parentKey ? "Открываем…" : "Добавить вложенный"}</button> : null}
              {onToggleReview ? <button className={`siteMenuReviewButton ${node.section?.is_review ? "active" : ""}`} type="button" onClick={() => onToggleReview(node.item, node.section)} disabled={reviewSaving || deletingMenuItem || savingEdit} aria-pressed={Boolean(node.section?.is_review)} aria-label={`Review: ${node.item.title}`} title={node.section?.is_review ? `Review включён: обзор казино «${node.item.title}». Нажмите, чтобы выключить.` : `Включить Review: название «${node.item.title}» будет брендом для обзора казино.`}><BookOpen size={13} /><span>Review</span></button> : null}
              {onDeleteMenuItem ? <button className="iconButton siteMenuDeleteButton" type="button" onClick={() => onDeleteMenuItem(node.item)} disabled={deletingMenuItem || savingEdit || Boolean(adoptingParentKey)} title={`Удалить «${node.item.title}»${hasChildren ? " и все вложенные пункты" : ""}`} aria-label={`Удалить пункт меню ${node.item.title}`}><Trash2 size={14} /></button> : null}
            </div>
              ...
              <ul className="siteMenuNestedPages" aria-label={`Страницы в пункте ${node.item.title}`}>
                {nestedPages.map((page) => {
                  ...
                  return <li className={page.status === "published" ? "siteMenuPublishedPageRow" : undefined} key={page.id} title={failureDetails || undefined}>
                    ...
                    {onRetryPage && page.status === "publication_failed" ? <button className="button compact primary" type="button" onClick={() => onRetryPage(page)} disabled={retryingNestedPageId === page.id}><Send size={13} /> {retryingNestedPageId === page.id ? "Проверяем…" : "Отправить ещё раз"}</button> : null}
