...
with sync_playwright() as p:
    ...
    page.evaluate('localStorage.setItem("sites-table-preferences:user",JSON.stringify({geoFilter:"AZ_AZ"}))')
    page.goto(base+'/sites')
    geo=page.get_by_label('Фильтр сайтов по GEO')
    expect(geo).to_have_value('AZ')
    assert geo.locator('option').all_text_contents()==['Все GEO','🇦🇿 AZ','🇺🇦 UA','🇺🇸 US']
