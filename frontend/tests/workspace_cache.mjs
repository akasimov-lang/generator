...
const live = workspaceRequestCache(async () => ++calls);
await live.api('/sites/a/network');
const freshNetwork = await live.api('/sites/a/network?refresh=true', {cache:'no-store'});
assert.equal(await live.api('/sites/a/network'), freshNetwork);
const ownConfig = await live.api('/auto-reglue/projects/a');
assert.equal(await live.api('/auto-reglue/projects/a'), ownConfig);
assert.notEqual(await live.api('/auto-reglue/projects/a', {cache:'no-store'}), ownConfig);
assert.notEqual(await live.api('/auto-reglue/projects/a/preview', {method:'POST'}), ownConfig);
