...
with sync_playwright() as p:
 ...
 expect(page.get_by_role('heading',name='Автопереклей проекта',exact=True)).to_be_visible()
 expect(page.get_by_role('button',name='Переклеить на резервный домен',exact=True)).to_have_count(0)
 link.click()
 expect(page.locator('#auto-task-preview')).to_be_visible()
 page.set_viewport_size({'width':390,'height':844})
 assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
 page.goto(base+'/project-auto-reglue/betonredczech.com/')
 expect(page.get_by_role('heading',name='Автопереклей проекта',exact=True)).to_be_visible()
 expect(page.get_by_label('Формат имени поддомена',exact=True)).to_have_value('hyphen')
 assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
 page.get_by_role('link',name='Переклей',exact=True).click()
 expect(page.get_by_role('heading',name='Автопереклей проекта',exact=True)).to_have_count(0)
