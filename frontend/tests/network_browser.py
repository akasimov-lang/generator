...
with sync_playwright() as p:
 ...
 expect(page.get_by_label('unused.reserve.test: поддомен',exact=True)).to_be_checked()
 expect(page.get_by_text('Поддомен новорега',exact=False)).to_be_visible()
 expect(page.get_by_text('Не был Main',exact=True)).to_be_visible()
 expect(page.get_by_label('Тип домена unused.reserve.test',exact=True)).to_have_count(0)
 page.get_by_label('Путь фейковой главной',exact=True).fill('test1')
 ...
 print('PASS: reserve → alternates → manual reglue without check; unsaved markup blocks launch; drafts survive tabs; stale markup conflict; history table; no browser errors.')
